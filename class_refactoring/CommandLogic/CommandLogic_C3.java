...
public class CommandLogic extends AbstractLogic<EntityTO> {
    ...
    public Page<CommandTO> search(final String keyword, final Pageable pageable) {
        List<Implementation> result = implementationDAO.findByTypeAndKeyword(
                IdRepoImplementationType.COMMAND, keyword);
                ...
                map(command -> {
                    try {
                        return new CommandTO.Builder(command.getKey()).
                                args(ImplementationManager.emptyArgs(AuthContextUtils.getDomain(), command)).
                                build();
                    } catch (Exception _) {
                        return null;
                    }
    ...
    public String run(final CommandTO command) {
        ...
        Command runnable;
        try {
            runnable = (Command) ImplementationManager.build(
                    impl,
        ...
        if (command.getArgs() != null) {
            try {
                Set<ConstraintViolation<CommandArgs>> violations = validator.validate(command.getArgs());
                ...
                sce.getElements().add("Could not validate " + command.getArgs().getClass().getName());
                throw sce;
            }
        }
        ...
        try {
            ...
            sce.getElements().add("Could not run " + command.getKey());
            throw sce;
        }
    }
        ...
        if (args != null) {
            for (Object arg : args) {
                if (arg instanceof String string) {
                    key = string;
                } else if (arg instanceof CommandTO command) {
                    key = command.getKey();
                }
            }
        }
